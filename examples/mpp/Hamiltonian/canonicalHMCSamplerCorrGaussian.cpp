...
public:
     ...
     */
    GaussianLogPost(realVectorType const& mu,realMatrixType const& sigmaInv)
    ...
    {
        val = -0.5*(mMu-q).transpose()*mSigmaInv*(mMu-q);
    }
...
private:
    ...
    realMatrixType mSigmaInv; /**< the inverse of the covariance matrix */
};
...
int main(void)
{
    ...
    typedef mpp::Hamiltonian::GaussKineticEnergy<double> kinEngType;
    ...
    realMatrixType mat = realMatrixType::Random(numParams,numParams);
    ...
    std::string const rootPathStr("./exampleCanonicalSamplerCorrGaussian");
